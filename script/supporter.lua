...
local default_bot_name = mod_defines.unit.name
...
function mining_drone:oof()
  local position =
    self.entity.surface.find_non_colliding_position(self.entity.name, self.entity.position, 0, 0.1, false)
...
function mining_drone:process_failed_command()
  ...
  if self.fail_count == 2 then
    self.entity.ai_settings.path_resolution_modifier = 2
  end
  if self.fail_count == 4 then
    self.entity.ai_settings.path_resolution_modifier = 3
  end

...
function gui_update(player)
  local gui = mod_gui.get_frame_flow(player).new_silo_gui_frame
  if not gui then
    return
  end
  gui.clear()
  ...
  if table_size(launched) > 0 then
    ...
    for name, bool in pairs(get_tracked_items()) do
      ...
      local count = launched[name] or 0
      if name:match("patreon%-tier") ~= nil then
        storage = storage + count
      elseif item and count then
    ...
    if storage > 0 then
      local label =
        item_table.add {
        type = "label",
        caption = {"", items["patreon-satellite"].localised_name, {"colon"}}
      }
      label.style.font = "default-semibold"
      item_table.add {
        type = "label",
        caption = {"", util.format_number(storage), " / ", remote.call("patreon_rocket", "get_totals")}
      }
    end
  else
...
function toggle_frame(player)
  ...
  local frame = gui.new_silo_gui_frame

  if gui.silo_gui_frame and gui.silo_gui_frame.valid then
...
get_sprite_button = function(player)
  ...
  local old_button = button_flow.silo_gui_sprite_button
  if old_button and old_button.valid then
    old_button.destroy()
  end
  local button = button_flow.new_silo_gui_sprite_button
  if not button then
    ...
    if item then
      button =
        button_flow.add {
        type = "sprite-button",
        name = "new_silo_gui_sprite_button",
...
local on_rocket_launched = function(event)
  ...
  local total_counts = remote.call("patreon_rocket", "get_totals")
  ...
  local counted = 0
  local launched = force.items_launched
  ...
  --They finished, lets just check if we should turn on the research queue for them...
...
local on_gui_click = function(event)
  ...
  if gui.name == "new_silo_gui_sprite_button" then
    toggle_frame(player)
    return
  end
end
    ...
    "new_silo_script",
    {
      ...
      add_tracked_item = function(item_name)
        if type(item_name) ~= "string" then
          error("Value for 'add_tracked_item' must be a string")
        end
        if not game.item_prototypes[item_name] then
          error("Item to add is not a valid item: " .. item_name)
        end
      ...
      remove_tracked_item = function(item_name)
        if type(item_name) ~= "string" then
          error("Value for 'remove_tracked_item' must be a string")
        end
...
silo_script.add_commands = function()
  commands.add_command(
    "new-toggle-rockets-sent-gui",
    {"gui-silo-script.toggle-rockets-sent-command-help"},
    function()
      local player = game.player
      local button = get_sprite_button(player)
      local set_to = not button.visible
      button.visible = set_to
      local frame = mod_gui.get_frame_flow(player).new_silo_gui_frame
